        ...
        mandatoryQuestions.forEach(question => {
            ...
            if (questionLabel && !questionLabel.classList.contains('asterisk-injected')) {
                ...
                if (!questionLabel.querySelector('.required-asterisk')) {
                    ...
                    // Structure : <h3>.question-text > [badge] [question-number] [question-code] [texte]
                    // On veut insérer l'astérisque AVANT le premier noeud texte significatif

                    // Chercher le dernier élément structurel (badge, question-number, question-code)
                    const lastStructuralElement = questionLabel.querySelector('.question-code') ||
                                                   questionLabel.querySelector('.question-number') ||
                                                   questionLabel.querySelector('.fr-badge');
